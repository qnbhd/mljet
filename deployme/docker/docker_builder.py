...
from deployme.utils import copy_template_files, call, copy_model, get_random_name
...
def build_image(model_path, image_name, base_image):
    ...
    command = ['docker', 'build', ] + \
              ...
              ['--tag', image_name] + [str(project_path)]
    call(command)


def run_image(image_name, container_name=None, port=5000):
    """Run builded Docker image."""
    container_name = container_name if container_name else get_random_name()
    command = ['docker', 'run'] + ['-p', f'{port}:5000'] + ['--name', container_name] + [image_name]
    call(command)
...
def deploy_to_docker(
    ...
    run_image(image_name, container_name=container_name)
